...
public struct PeakDetectionPacket {
...
public class PeakDetector {
    ...
    public func process(value:Double) -> PeakDetectionPacket? {
        ...
        if (_trimBuffer()){
            ...
            //then detect peaks
            return detectPeak(from: _buffer)
        }
    ...
    //this adds to buffer, then buffer is passed into the peak detection method
    public func process(packet:[Double]) -> PeakDetectionPacket? {
    ...
    public func process(yDataSet:[Double]) -> PeakDetectionPacket? {
        ...
        if (yDataSet.count != _bins){
            ...
            return detectPeak(from: yDataSet)
        }
    }
    ...
    public func detectPeak(from rawSamples:[Double]) -> PeakDetectionPacket? {
        ...
        //return the peaks, averaged filter, and deviation filter
        return PeakDetectionPacket(peaks: _peaks, averagedValues: _avgFilter, deviationValues: _devFilter)
